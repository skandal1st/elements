"""
Dependencies для модуля Договора.
"""
...
def require_contracts_roles(allowed_roles: Sequence[str]):
    """Проверяет роль в модуле contracts."""

    def _checker(user: User = Depends(get_current_user)) -> User:
        if user.is_superuser:
            return user
        role = user.get_role("contracts")
        if not role:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Нет доступа к модулю Договора",
            )
